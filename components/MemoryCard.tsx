...
const TypewriterPrompt: React.FC<{ tag?: string; subTag?: string; typewriterEnabled?: boolean }> = ({ tag, subTag, typewriterEnabled }) => {
  ...
  return (
    <div className="min-h-[2rem] overflow-hidden text-center text-sm text-black font-serif transition-all duration-300 whitespace-pre-wrap break-normal hyphens-auto">
      {displayedText}
    </div>
  );
};
      ...
      >
        ...
        <div className="w-full flex flex-col items-center relative z-10">
          <div className="flex items-center justify-center gap-2 sm:gap-3 mb-3 sm:mb-4">
            ...
            <h3 className={`${isDesktop ? "text-5xl" : "text-2xl"} font-bold text-black text-center leading-tight drop-shadow-sm`}>
              To: {memory.recipient}
            </h3>
          </div>
          
          {memory.sender && (
            <p className={`${isDesktop ? "text-2xl" : "text-base"} italic text-black opacity-75 mb-3 sm:mb-4 text-center font-light tracking-wide`}>
              From: {memory.sender}
            </p>
          )}
        ...
        <div className="w-full flex-1 flex flex-col justify-center items-center my-4 sm:my-8 relative z-10">
          <div className={`${isDesktop ? "text-5xl" : "text-base"} font-serif text-center text-black leading-relaxed break-words hyphens-none px-3 sm:px-4`}>
        ...
        <div className="w-full flex flex-col items-center relative z-10">
          ...
          <div className="flex flex-col items-center gap-1 sm:gap-2">
            <span className={`${isDesktop ? "text-xl" : "text-xs"} text-black opacity-75 font-medium text-center tracking-wide`}>
              {dateStr} • {dayStr} • {timeStr}
            </span>
            <span className={`${isDesktop ? "text-base" : "text-xs"} text-black opacity-60 font-light text-center capitalize`}>
              {effectiveColor}
            </span>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start">
                <h3 className="text-xl font-bold text-black break-words overflow-hidden">
                  {memory.animation && memory.animation !== "none" && (
              ...
              {memory.sender && <p className="mt-1 text-md italic text-black break-words overflow-hidden">From: {memory.sender}</p>}
              <hr className="my-2 border-black" />
            </div>
            <div className="text-xs text-black text-center font-normal relative z-10">
              {dateStr} | {dayStr}
            </div>
          ...
          >
            ...
            <h3 className="text-lg italic text-black text-center relative z-10">if only i sent this</h3>
