...
const ScrollableMessage: React.FC<{ children: React.ReactNode; style?: React.CSSProperties }> = ({ children, style }) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      setNeedsScroll(containerRef.current.scrollHeight > containerRef.current.clientHeight);
    }
  }, [children]);
  ...
  return (
    <div
      ref={containerRef}
      className={`flex-1 overflow-y-auto text-sm text-[var(--text)] whitespace-pre-wrap break-words pt-2 ${needsScroll ? "cute_scroll" : ""}`}
      style={style}
    >
      {children}
    </div>
